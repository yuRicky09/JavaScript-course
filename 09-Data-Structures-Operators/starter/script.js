...
btn.addEventListener('click', function () {
  ...
  console.log(newText);
  for (let [i, textItem] of newText.entries()) {
    // console.log(textItem);
    let [first, second] = textItem.toLowerCase().trim().split('_');
    textItem = first + second[0].toUpperCase() + second.slice(1);
    // textItem = textItem.replace('_', '');
    console.log(textItem);
    textItem = textItem.padEnd(20, '·') + '🚗'.repeat(i + 1);
    console.log(textItem);
    arr.push(textItem);
  }
  console.log(arr);
